...
#define false 0
#define true 1
void Initialize(int distance[V][V], int graph[V][V])
{
	for (int i = 0;i < V;i++)
		for (int j = 0;j < V;j++)
			if (i == j)
...
void fiilGraph(int graph[][V]) {
	for (int i = 0; i < V; i++)
		for (int j = 0; j < V; j++)
			scanf_s("%d", &graph[i][j]);
}
int Route(int from, int to, int graph[][V], int visited[V]) {
	if (from == to)
		return false;
	if (graph[from][to])
		return true;
	for (int i = 0;i < V;i++)
	{
		if (graph[from][i] && !visited[i]) {
			visited[i] = 1;
			if (Route(i, to, graph, visited))
				return true;
		}
	}
	return false;
}
void FloydWarshall(int distance[][V], int vertices) {
	int from, to, via;
